...
function ConfirmForgotPasswordFormPrescriber({
  ...
  const [email, _setEmail] = useState<string>(emailForgotPassword);
  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      ...
      if (dataS.message) {
        alert(`Não foi possível fazer essa operação: ${dataS.message}`);
        setLoading(false);
      } else {
        alert(
      ...
      >
        ...
        <label class="w-full">
          <div class="label pb-1">
            <span class="label-text text-xs text-[#585858]">Código</span>
          </div>
          <input
        ...
        <label class="w-full">
          <div class="label pb-1">
            <span class="label-text text-xs text-[#585858]">Nova Senha</span>
          </div>
          <input
            type="password"
            class="input rounded-md text-[#8b8b8b] border-none w-full"
