...
const PrescriberUpdateTreatmentModal = ({ onFinished }: Props) => {
  ...
  const getActiveTreatmentByPatient = async (accessToken: string) => {
    setUpdating(true);
    const response = await invoke[
      "deco-sites/ecannadeco"
    ].actions.getTreatments({ token: accessToken });
    const currentTreatment = response?.find((t) => t.isActive) || null;
    setUpdating(false);
    if (currentTreatment) {
      setCurrentTreatment(currentTreatment as Treatment);
      setNewTreatment(currentTreatment as Treatment);
      setNewMedication(currentTreatment.medications as Medications);
      if (currentTreatment.treatmentJourneyStatus === "STARTED_TREATMENT") {
        setHasMedication(true);
        setStartedTreatment(true);
      } else if (
        currentTreatment.treatmentJourneyStatus === "BOUGHT_MEDICATION"
      ) {
        setHasMedication(true);
      }
  ...
  useEffect(() => {
    const accessToken = IS_BROWSER
      ...
      : "";
    getActiveTreatmentByPatient(accessToken);
  }, []);
  ...
  const handleSubmit = async () => {
    ...
    const allowed_pros = [doctorEmail];
    formData.append("allowed_pros", JSON.stringify(allowed_pros));
      ...
      <div class="flex flex-col p-10 gap-3 max-h-[90%] max-w-[90%] overflow-scroll bg-[#EDEDED] rounded-xl">
        ...
        <div class="flex flex-col gap-4">
          <label class="form-control w-full flex flex-row items-center gap-2">
            ...
            />
            <span>Já comprei/possuo a medicação</span>
          </label>
          ...
          <label class="form-control w-full flex flex-row items-center gap-2">
            ...
            />
            <span>Já estou tomando a medicação</span>
          </label>
          ...
          <label class="form-control w-full flex flex-row items-center gap-2">
            <input
              ...
              class="checkbox checkbox-xs"
              onChange={() => {
                setShareDoctor(!shareDoctor);
              }}
            />
            <span>Quero compartilhar meu acompanhamento com meu médico</span>
          </label>
        </div>
          ...
          <div class="p-3 rounded-md bg-[#dbdbdb] w-full">
            <label class="form-control w-full">
              ...
              <input
                ...
                value={doctorEmail}
                onChange={(e) => {
                  setDoctorEmail(e.currentTarget.value);
                }}
              />
            </label>
          </div>
