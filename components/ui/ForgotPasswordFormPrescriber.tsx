...
function ForgotPasswordFormPrescriber({
  ...
  const handleSubmit = async (e: Event) => {
    ...
    try {
      ...
      ].actions.forgotPasswordPrescriber({ email });
      ...
      if (dataS.errors && dataS.errors.length > 0) {
        alert(
          "Não foi possível fazer operação. Verifique as informações fornecidas e tente novamente.",
        );
        setLoading(false);
      } else {
        if (IS_BROWSER) {
        ...
        setLoading(false);
        window.location.href = "/prescritor/confirmar-recuperar-senha";
      }
    } catch (e) {
...
export default ForgotPasswordFormPrescriber;
