...
function PublicProfileComponent({ publicProfile }: Props) {
  ...
  return publicProfile.plan === "DEFAULT" ? (
    <PageWrap>
      <div class="flex flex-col gap-4 items-center bg-[#ffc040] rounded-lg p-5">
        <Icon id="Warn" size={24} />
        <div class="text-center">
          Atenção: O plano de serviço do paciente está desativado. Ative o plano
          em "Minha Conta" para continuar disponibilizando suas informações com
          segurança!
        </div>
        <a href="/entrar" class="btn btn-primary">
          Acessa Minha Conta
        </a>
      </div>
    </PageWrap>
    ...
    <PageWrap>
      {publicProfile.name ? (
        <div class="flex flex-col items-center gap-5 max-w-[95%]">
          {publicProfile.plan == "CARD_PARTNER" ||
          (publicProfile.plan == "CARD_ASSOCIATED" &&
            ...
            {publicProfile?.name && publicProfile?.cpf ? (
              ...
              <div class="p-3 flex flex-col items-center text-[#5e5e5e] text-sm bg-[#cecece] rounded-md gap-3">
                <span class="text-center">
                  Informação Pendente: Paciente deve fazer cadastro com email{" "}
                  <span class="font-bold">{publicProfile?.email}</span> e
                  atualizar dados médicos / pessoais.
                </span>
                <a href="/cadastrar" class="btn btn-primary btn-xs text-white">
            ...
            {publicProfile?.association &&
              ...
              publicProfile?.association.cnpj !== "47774121000154" && (
                <div class="flex flex-col items-center mt-4">
                  ...
                  <span class="text-[#5B5B5B] font-semibold text-sm">
                    CNPJ{" "}
                    {publicProfile?.association.cnpj.replace(
                      /^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2})$/,
                      "$1.$2.$3/$4-$5"
                    )}
                  </span>
          ...
          <div class="bg-gray-200 p-2 rounded-md leading-3 flex items-center gap-3">
            ...
            <span class="text-[10px] text-[#333]">
              Este é um serviço feito para que pacientes de cannabis medicinal
              disponibilizem seus documentos de forma facilitada, caso haja
              qualquer necessidade de fiscalização. A condição de paciente se
              respalda exclusivamente na validade dos documentos
              médicos/jurídicos que porventura se encontrem anexados e
              disponíveis nesta página. O ecanna não se responsabiliza por
              falsidade ideológica, falta de documentos ou prestação de
              informação falsa por parte dos seus usuários.
            </span>
          </div>
        </div>
      ) : (
